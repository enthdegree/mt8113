...
// MT8113 specific MMC functions and definitions
#define PAD_TUNE0 ((uint32_t*) (0x11230000 + (0xEC/4)))
#define DAT_RD_DLY0 ((uint32_t*) (0x11230000 + (0xF0/4)))
#define DAT_RD_DLY1 ((uint32_t*) (0x11230000 + (0xF4/4)))
#define PATCH_BIT0 ((uint32_t*) (0x11230000 + (0xB0/4)))
#define PATCH_BIT1 ((uint32_t*) (0x11230000 + (0xB4/4)))
#define PATCH_BIT2 ((uint32_t*) (0x11230000 + (0xB8/4)))
//#define EMMC50_CFG0 ((uint32_t*) (0x11230000 + (0x208/4)))
#define EMMC50_BLOCK_LEN ((uint32_t*) (0x11230000 + (0x228/4)))
#define SDC_ADV_CFG0 ((uint32_t*) (0x11230000 + (0x64/4)))
#define MSDC_CMD_R1_RESP       (1 << 7)
#define MSDC_CMD_R1B_RESP      (7 << 7)
#define MSDC_CMD_R2_RESP       (2 << 7)
#define MSDC_CMD_R3_RESP       (3 << 7)
int mt8113_send_cmd(uint8_t cmd_idx, uint32_t arg, uint32_t flags) {
    while(*SDC_STS & 0x2);
    *MSDC_INT = 0xFFFFFFFF;
    *SDC_ARG = arg;
    *SDC_CMD = cmd_idx | flags;
    uint32_t timeout_val = 1000000;
    while (timeout_val-- > 0) if (*MSDC_INT & MSDC_INT_CMDRDY) return 0; 
    return -1;
}

int mt8113_wait_card_ready(void) {
    for (int i = 0; i < 1000; i++) {
        if (mt8113_send_cmd(13, 0x00010000, MSDC_CMD_R1_RESP) == 0) {
            uint32_t state = (*SDC_RESP0 >> 9) & 0xF;
            if (state == 4) return 0;
        }
...
void mt8113_mmc_clear_fifo(void) {
    *MSDC_FIFOCS = 0x80000000;
    while (*MSDC_FIFOCS & 0x80000000);
}

void mt8113_mmc_init(void) {
    int retry;
    
    // Controller reset
    *MSDC_CFG |= 0xF;
    for (retry = 5000; retry > 0; retry--) {
        if ((*MSDC_CFG & 0x4) == 0) break;
    }

    mt8113_mmc_clear_fifo();
    ...
    *PATCH_BIT0 = 0x403C0046;
    *PATCH_BIT1 = 0xFFFF4289;
    *PATCH_BIT2 = 0x10008000;
    
    // eMMC 5.0 config
    *EMMC50_CFG0 |= 0x10;
    *EMMC50_BLOCK_LEN &= 0xFCFFFFFF;
    *SDC_ADV_CFG0 |= 0x100000;
    
    // IO config
    *MSDC_IOCON |= 0xE;
    *PATCH_BIT0 |= 0x40000000;
    *MSDC_CFG &= 0xFE7FFFFF;
    ...
    *MSDC_CFG &= 0xFFBFFFFF;
    *PATCH_BIT2 |= 0x10000000;
    
    *MSDC_CFG = (*MSDC_CFG & 0xFFC000FF) | 0x18500;
    
    // Wait for clock stable with timeout
    for (int i = 0; i < 100000; i++) if (*MSDC_CFG & 0x80) break;

    *SDC_CFG &= 0x00E7FFFD; // Timeout config
    *MSDC_INT = *MSDC_INT; // Interrupts
    *MSDC_INTEN = 0x738; 
    mt8113_send_cmd(0, 0, 0); // CMD0 - GO_IDLE_STATE
    for (retry = 100; retry > 0; retry--) {
        if (mt8113_send_cmd(1, 0x40FF8080, MSDC_CMD_R3_RESP) == 0) { // CMD1 - SEND_OP_COND
            if (*SDC_RESP0 & 0x80000000) break;
        }
        for (volatile int d = 0; d < 10000; d++);
    }
    mt8113_send_cmd(2, 0, MSDC_CMD_R2_RESP); // CMD2 - ALL_SEND_CID
    mt8113_send_cmd(3, 0x00010000, MSDC_CMD_R1_RESP); // CMD3 - SET_RELATIVE_ADDR
    mt8113_send_cmd(7, 0x00010000, MSDC_CMD_R1B_RESP); // CMD7 - SELECT/DESELECT_CARD
    
    // Increase clock to 25MHz
    *MSDC_CFG = (*MSDC_CFG & 0xFFC000FF) | (0x4 << 8);
    for (int i = 0; i < 100000; i++) {
        if (*MSDC_CFG & 0x80) break;
    }
    
    mt8113_wait_card_ready();
    mt8113_mmc_clear_fifo(); // Drain any leftover data in FIFO from init sequence
    while ((*MSDC_FIFOCS & 0xFF) > 0) {
        (void)*MSDC_RXDATA;
    }
    *MSDC_INT = 0xFFFFFFFF;
    return;
}
// End of MT8113 specific code
